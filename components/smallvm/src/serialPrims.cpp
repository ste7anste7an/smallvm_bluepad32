...
static void serialOpen(int baudRate) {
	if (isOpen) serialClose();
		...
		*/
		...
		if (getESPVersion() ==2) {
			//pinMode(7, OUTPUT);
			//gpio_iomux_out(GPIO_NUM_7, SIG_GPIO_OUT_IDX, false);
			//gpio_matrix_in(GPIO_NUM_7, GPIO_MATRIX_CONST_ONE_INPUT, false);
			pinMode(7, OUTPUT);        // Sets GPIO22 as input
			digitalWrite(7, OUTPUT);     // Optional: clear any output
...
//sodb
OBJ primSerialMillis(int argCount, OBJ *args) {
	int value = millis();
